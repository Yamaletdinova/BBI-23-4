...
namespace ЛР6_1_защита
{
    internal class Program
    {
        ...
        {
            int step = uchastniki.Length / 2;
            while (step > 0)
            {
                for (int i = step; i < uchastniki.Length; i++)
                {
                    Character temp = uchastniki[i];
                    int j = i;
                    while (j >= step && uchastniki[j - step].Rezult > temp.Rezult)
                    {
                        uchastniki[j] = uchastniki[j - step];
                        j = j - step;
                    }
                    uchastniki[j] = temp;
                }
